...
const uint64_t ORDER_INDEX_PREAMBLE = 8 * 3;
...
void write_pages(filemap_ordered_list_t* list, uint64_t next, uint64_t min,
		 uint64_t length) {
  // next, min, and length
...
filemap_ordered_list_t filemap_ordered_list_new(char* order_list, int page_size,
						int overwrite) {
...
// locks data
int freelist_insert(filemap_t* filemap, uint64_t freelist, uint64_t block,
		    uint64_t size) {
...
filemap_object filemap_cpy(filemap_t* filemap, filemap_partial_object* res) {
  ...
  if (res->exists) {
    ...
    // read lengths
    uint64_t sum = 0;  // use sum to decode skips into lengths
...
uint64_t filemap_page_insert(filemap_ordered_list_t* list, uint64_t length,
			     uint64_t item_order, uint64_t data_pos) {
  uint64_t i = 0;
  ...
  if (i == length) {  // length == 0 or end of list
    ...
    vector_t shift = filemap_page_read(list, length - i);
...
// TODO: insert before first page and adjust min
...
vector_t filemap_ordered_page(filemap_ordered_list_t* list, uint64_t start_page,
			      unsigned pages) {
...
void filemap_list_remove(filemap_t* filemap, filemap_list_t* list,
			 filemap_partial_object* obj) {
  mtx_lock(&list->lock);
...
void filemap_list_update_ref(filemap_list_t* list, filemap_partial_object* partial,
			 filemap_object* obj) {
